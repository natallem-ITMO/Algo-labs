...
struct Node {
    ...
    Node *p, *l, *r;
    int sum;
    Node(int key, Node *p = nullptr, Node *l = nullptr, Node *r = nullptr, int sum = 0) :
    /*key(key),*/ p(p), l(l), r(r) , sum(sum) {}
...
void recalc(Node * node){
    node->sum = get_sum(node->l) + get_sum(node->r) + 1;
}
...
struct SplayTree {
    ...
    Node *splay(Node *root, int index) {
        int sum_left = get_sum(root->l);
        if (root == nullptr || sum_left + 1 == index)
            return root;
        //key это сумма в левом + 1

        if (sum_left + 1 > index) {
            if (root->l == nullptr) return root;
            int sum_left_in_left = get_sum(root->l->l);

            if (sum_left_in_left+1 > index) {
