/*#include <iostream>
#include <vector>
#include <string>
#include <stdio.h>

using std::vector;
using std::max;
using std::min;
using std::cout;
using std::cin;
using std::endl;
using std::string;
int firstIndex;
int lastIndex;
int MAX = INT32_MAX;
int noIncline = INT32_MIN;

int getRs(int node) {
    return node * 2 + 2;
}

int getLs(int node) {
    return getRs(node) - 1;
}

void push(vector<int> &vec, int node) {
    if (node >= firstIndex)
        return;
    int ls = getLs(node);
    int rs = getRs(node);
    vec[ls] = max(vec[ls], vec[node]);
    vec[rs] = max(vec[rs], vec[node]);
}

void setMin(vector<int> &vec, int valueMin, int node, int l, int r, int L, int R) {
    if (L >= r || R <= l) return;
    if (L >= l && R <= r) {
        if (vec[node] < valueMin) vec[node] = valueMin;
        return;
    }
    push(vec, node);
    int ls = getLs(node);
    int rs = getRs(node);
    int m = (L + R) / 2;
    setMin(vec, valueMin, ls, l, r, L, m);
    setMin(vec, valueMin, rs, l, r, m, R);
    vec[node] = min(vec[ls], vec[rs]);
}


int findMin(vector<int> &vec, int node, int l, int r, int L, int R) {
    push(vec, node);
    if (L >= r || R <= l) {
        //int k = 0;
        return MAX;
    }

    if (L >= l && R <= r) {
        return vec[node];
    }

    int ls = getLs(node);
    int rs = getRs(node);
    int m = (L + R) / 2;
    int lsRes = findMin(vec, ls, l, r, L, m);
    int rsRes = findMin(vec, rs, l, r, m, R);
    return min(lsRes, rsRes);
}

int findMinIndex(vector<int> &vec, int node) {
    push(vec, node);
    if (node >= firstIndex) return node;
    if (vec[getLs(node)] == vec[node]) findMinIndex(vec, getLs(node));
    else findMinIndex(vec, getRs(node));
}

int main() {
    freopen("rmq.in", "r", stdin);
    freopen("rmq.out", "w", stdout);

    int n;
    int m;
    scanf("%d", &n);

    int amountOfLeafs = 1;
    int power = 0;
    int amountOfVertices = 0;
    while (n > amountOfLeafs) {
        amountOfVertices += amountOfLeafs;
        amountOfLeafs = (amountOfLeafs << 1);
        power++;
    }
    amountOfVertices += amountOfLeafs;
    firstIndex = (1 << (power)) - 1;
    lastIndex = firstIndex + n - 1;
    vector<int> vec(amountOfVertices, MAX);
    for (int i = firstIndex; i <= lastIndex; i++) {
        vec[i] = noIncline;
    }

    for (int i = firstIndex - 1; i >= 0; i--) {
        vec[i] = min(vec[2 * i + 1], vec[2 * i + 2]);
    }
    scanf("%d", &m);
    //string command;
    //vector<string> result;
    bool suc = 1;
    vector<int> ll;
    vector<int> rr;
    vector<int> mm;

    for (int i = 0; i < m; i++) {
        int l, r, min;
        scanf("%d%d%d", &l, &r, &min);
        ll.push_back(l);
        rr.push_back(r);
        mm.push_back(min);
        int settedMin = findMin(vec, 0, l - 1, r, 0, amountOfLeafs);
        if (settedMin < min) setMin(vec, min, 0, l - 1, r, 0, amountOfLeafs);
        else if (settedMin > min) {
            suc = 0;
            goto m;
        }
    }
    for (int i = 0; i < m; i++) {
        int foundMin = findMin(vec, 0, ll[i] - 1, rr[i] , 0, amountOfLeafs);
        if (foundMin != mm[i]) {
           // printf("fuck");
            suc = 0;
            goto m;
        }
    }
    for (int i = 0; i < firstIndex; i++) {
        push(vec, i);
    }
m:
    if (suc==0) printf("inconsistent");
    else {
        printf("consistent\n");
        for (int i = firstIndex; i < firstIndex + n; i++){
            printf("%d ", vec[i]);
        }
    }

//printf("min=%d max=%d", noIncline, MAX);
return 0;
}
*/

