...
edge *makeEdge(int start, int end, int capacity, int cost, int num, vector<vector<edge *>> &graph) {
...
//    std::ofstream cout("a.out");
    ...
    for (int i = 0; i < m; i++) {
        ...
        cin >> a >> b >> cost;
        if (a == b) continue;
        makeEdge(a, b, 1, cost, i + 1, graph);
        makeEdge(b, a, 1, cost, i + 1, graph);
    }
    makeEdge(start, 1, k, 0, -1, graph);
    edge *endEdge = makeEdge(n - 2, end, k, 0, -1, graph);
    long long res = findMaxFlowMinCost(graph, start, end);
    if (endEdge->flow != k) {
        cout << -1;
        return 0;
    }
    cout << fixed << setprecision(5) << (double) res / k << endl;
    for (int i = 0; i < k; i++) {
        vector<int> path;
        int curVertex = 1;
        while (curVertex != n - 2) {
            for (auto &ed : graph[curVertex]) {
                if (ed->flow == 1 && ed->numRoad > 0) {
                    ed->flow = 0;
                    path.push_back(ed->numRoad);
                    curVertex = ed->end;
                    break;
                }
            }
