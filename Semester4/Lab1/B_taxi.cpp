#include <vector>
#include <iostream>
#include <algorithm>
#include <string>
#include <fstream>

bool debug = false;

using namespace std;


inline int parseTime(string &s) {
    int h = (s[0] - '0') * 10 + (s[1] - '0');
    int m = (s[3] - '0') * 10 + (s[4] - '0');
    return 60*h + m;
}

void showMatching(vector<int> &t, int i) {
    printf("matching %d : \n", i);
    for (int i = 0; i < t.size(); i++) {
        printf("%d -> %d\n", i, t[i]);
    }
}

bool dfs(int i, vector<vector<int>> &edges, vector<int> &matching, vector<bool> &visited) {

    if (visited[i]) return false;
    visited[i] = true;
    for (int j : edges[i]) {
        if (matching[j] == -1 || dfs(matching[j], edges, matching, visited)) {
            matching[j] = i;
            return true;
        }
    }
    return false;
}

vector<int> find_matching(vector<vector<int>> &edges, int n, int m) {   // A | B
    //return max matching to B
    //m is for B. n is for A
    vector<int> matching(m, -1);
    vector<bool> visited(n);
    for (int i = 0; i < n; i++) {
        fill(visited.begin(), visited.end(), false);
        dfs(i, edges, matching, visited);
        if (debug) showMatching(matching, i);
    }
    return matching;
}

int calcDist(const pair<int, int> &a, const pair<int, int> &b) {
    return abs(a.first - b.first) + abs(a.second - b.second);
}

bool can_arrive(pair<int, pair<int, int>> &start, pair<int, pair<int, int>> &end) {
    return (start.first + calcDist(start.second, end.second) < end.first);
}

int main() {

#ifdef MY_DEBUG
    debug = true;
//    std::ifstream cin("taxi.in");
//    std::ofstream cout("taxi.out");
#endif
    std::ifstream cin("taxi.in");
    std::ofstream cout("taxi.out");
    ios_base::sync_with_stdio(false);
    cin.tie(nullptr);
    cout.tie(nullptr);
    int M;
    cin >> M;
    vector<pair<int, pair<int, int>>> start(M);
    vector<pair<int, pair<int, int>>> end(M);
    vector<int> doNotEdge(M);
    for (int i = 0; i < M; i++) {
        string s;
        cin >> s;
        int x1, y1, x2, y2;
        cin >> x1 >> y1 >> x2 >> y2;
        start[i] = {parseTime(s), {x1, y1}};
        end[i] = {start[i].first+calcDist({x1, y1}, {x2, y2}), {x2, y2}};
    }
    //находим граф
    vector<vector<int>> edges(M);
    for (int i = 0; i < end.size(); i++) {
        for (int j = 0; j < start.size(); j++) {
            if (i == j) continue;
            if (can_arrive(end[i], start[j])) {
                edges[i].push_back(j);
            }
        }
    }
    auto t = find_matching(edges, M, M);
    if (debug) showMatching(t, -1);
    int ans = 0;
    for (int i = 0; i < t.size(); i++) {
        if (t[i] == -1) ++ans;
    }
    cout << ans;
    return 0;
}



